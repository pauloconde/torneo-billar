      ...
      <div className="min-h-screen bg-background p-4">
        <div className="max-w-6xl mx-auto">
          <div className="text-center py-8">
            ...
            <p className="mt-4 text-muted-foreground">Cargando detalle de partida...</p>
          </div>
        </div>
      </div>
    )
        ...
        <Card className="mb-6 border-border">
          ...
          <CardContent>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
              ...
              <div className="text-center">
                <div className="flex items-center justify-center space-x-2 mb-2">
                  ...
                  <span className="text-xs text-muted-foreground uppercase">Bola Blanca</span>
                </div>
                <div className="space-y-1">
                  <h3 className="font-bold text-lg">{nombre1.nombres}</h3>
                  <h3 className="font-bold text-lg -mt-2">{nombre1.apellidos}</h3>
                </div>
                <div className="text-4xl font-bold text-white mt-2">{partida.carambolas1}</div>
                {resultadoPartida === "jugador1" && (
              ...
              <div className="text-center">
                <div className="flex items-center justify-center space-x-2 mb-2">
                  ...
                  <span className="text-xs text-muted-foreground uppercase">Bola Amarilla</span>
                </div>
                <div className="space-y-1">
                  <h3 className="font-bold text-lg">{nombre2.nombres}</h3>
                  <h3 className="font-bold text-lg -mt-2">{nombre2.apellidos}</h3>
                </div>
                <div className="text-4xl font-bold text-yellow-400 mt-2">{partida.carambolas2}</div>
                {resultadoPartida === "jugador2" && (
                  <Badge className="mt-2 bg-green-500 hover:bg-green-600">Ganador</Badge>
                )}
              </div>
            ...
            <div className="grid grid-cols-3 md:grid-cols-3 gap-4 pt-4 border-t border-border">
              <div className="text-center">
                <div className="text-2xl font-bold text-foreground">{partida.entradas1}</div>
                <p className="text-sm text-muted-foreground">Entradas</p>
              </div>
              <div className="text-center">
                <div className="text-2xl font-bold text-foreground">{partida.seriemayor1}</div>
                <p className="text-sm text-muted-foreground">Serie Blanca</p>
              </div>
              <div className="text-center">
                <div className="text-2xl font-bold text-yellow-400">{partida.seriemayor2}</div>
                ...
                <ResponsiveContainer width="100%" height="100%">
                  <LineChart data={dataEvolucion}>
                    ...
                    <Tooltip
                      content={({ active, payload, label }) => {
                        if (active && payload && payload.length) {
                          const data = dataEvolucion.find((d) => d.entrada === label)
                          return (
                            <div className="bg-background border border-border rounded-lg p-3 shadow-lg">
                              <p className="font-semibold">{`Entrada ${label}`}</p>
                              {payload.map((entry, index) => (
                                <div key={index}>
                                  <p style={{ color: entry.color }}>{`${entry.dataKey}: ${entry.value} (acumulado)`}</p>
                                  <p style={{ color: entry.color }} className="text-sm opacity-75">
                                    {`Esta entrada: ${index === 0 ? data.carambolas1 : data.carambolas2}`}
                                  </p>
                                </div>
                              ))}
                            </div>
                    ...
                    <Line
                      ...
                      stroke={colorBlanco}
                      strokeWidth={3}
                      dot={{ fill: "#aaa", strokeWidth: 1, r: 3, stroke: colorBlanco }}
                    ...
                    <Line
                      ...
                      strokeWidth={3}
                      dot={{ fill:"#C99011", strokeWidth: 1, r: 3, stroke: colorAmarillo }}
                      activeDot={{ r: 6, stroke: colorAmarillo, strokeWidth: 2 }}
                    />
                  </LineChart>
                </ResponsiveContainer>
              </div>
                  ...
                  {dataEvolucion.map((entrada, index) => (
                    <tr key={index} className="border-b border-border/50">
                      ...
                      <td className="text-center p-2 text-sm text-muted-foreground">
                        {entrada[partida.jugador1_data.nombre]} - {entrada[partida.jugador2_data.nombre]}
                      </td>
                    </tr>
                  ))}
                </tbody>
